        ...
        <div class="house-detail-container">
            ...
            <div class="basic-info-section">
                <h1 class="house-title">
                    {{ house.name }}
                    <el-tag effect="dark" type="success" v-if="!house.agentId"
                        style="margin-left: 10px; vertical-align: middle;">房东直租</el-tag>
                    <el-tag effect="dark" type="primary" v-else
                        style="margin-left: 10px; vertical-align: middle;">中介托管</el-tag>
                </h1>
            ...
            <div class="detail-section">
                ...
                <div class="detail-grid">
                    ...
                    <div class="detail-item" style="grid-column: span 3; justify-content: flex-start;">
                        <span class="label" style="margin-right: 20px;">房产证号</span>
                        <span class="value">{{ house.propertyCertificateNo || '暂无信息' }}</span>
                    </div>
                </div>
            </div>
            ...
            <!-- 3D看房区 -->
            <div class="description-section" v-if="house.model3dUrl || panoramaList.length > 0">
                <div class="section-title">3D看房</div>
                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                    <!-- 真3D模型漫游（优先显示） -->
                    <div class="save" v-if="house.model3dUrl" @click="open3DViewerDrawer">
                        <i class="el-icon-view"></i>
                        3D模型漫游
                    </div>
                    <!-- 360度全景（备用） -->
                    <div class="save" v-if="panoramaList.length > 0" @click="openPanoramaDrawer">
                        <i class="el-icon-view"></i>
                        360度全景
                    </div>
                </div>
            ...
            <div class="contract-section">
                <div class="section-title">合同下载</div>
                <div class="contract-links">
                    <div class="contract-link" @click="downloadContract('house_lease_contract.docx')">
                        <i class="el-icon-document"></i>
                        房屋租赁合同
                    </div>
                    <div class="contract-link" @click="downloadContract('house_renewal_contract.docx')">
                        <i class="el-icon-document"></i>
                        房屋续租合同
                    </div>
                    <div class="contract-link" @click="downloadContract('house_breach_contract.docx')">
                        <i class="el-icon-document"></i>
                        违约处理合同
                    </div>
                </div>
            ...
            <div class="action-bar">
                <div class="contact-info">
                    <div class="avatar">
                        <img :src="house.agentId ? house.agentAvatar : house.landlordAvatar" alt="头像">
                    </div>
                    <div class="info">
                        <div class="name">{{ house.agentId ? house.agentName : house.landlordName }}</div>
                        <div v-if="house.agentId" style="margin-top: 5px;">
                             <el-rate
                                :value="house.agentScore || 5.0"
                                disabled
                                show-score
                                text-color="#ff9900"
                        ...
                        <div class="role">
                            <el-tag size="mini" effect="dark" type="primary" v-if="house.agentId">中介托管</el-tag>
                            <div class="save" @click="fetchLandlordHouseList" v-else>
        ...
        <el-drawer title="预约看房" size="40%" :visible.sync="drawerHouseOrder" :direction="direction"
            ...
            <div class="house-order-drawer">
                <div class="contact-info">
                    ...
                    <div class="info">
                        <div class="name">{{ house.agentId ? house.agentName : house.landlordName }}</div>
                        <div class="role">{{ house.agentId ? '中介' : '房东' }}</div>
                    </div>
                </div>
                <!-- 步骤条 -->
                <div>
...
<!-- HouseDetail.vue 中的 3D 抽屉部分 -->
<el-drawer title="3D模型漫游" size="90%" :visible.sync="drawer3DViewer" :direction="direction" :before-close="close3DViewerDrawer">
    <div style="height: 100%; position: relative; background: #000;"> 
        <ThreeHouseViewer 
            v-if="drawer3DViewer && house.model3dUrl"
            :key="'3d-viewer-' + house.id"
            :model-url="'http://localhost:21090' + house.model3dUrl"
            :show-controls="true"
            :initial-position="{ x: 0, y: 1.6, z: 5 }"
            :move-speed="0.1"
            :sprint-multiplier="2.0"
            style="width: 100%; height: 85vh;" 
        />
        <el-empty v-else description="无3D模型"></el-empty>
    </div>
</el-drawer>
        <!-- 360度全景抽屉（备用） -->
        <el-drawer title="360度全景" size="80%" :visible.sync="drawerPanorama" :direction="direction" :before-close="closePanoramaDrawer">
            <div style="height: 100%;">
                <div v-if="panoramaList.length > 0">
                    <el-select v-model="currentPanoramaIndex" placeholder="选择房间全景">
                        <el-option v-for="(pano, index) in panoramaList" :key="index" 
                            :label="pano.name || `全景 ${index + 1}`" 
                            :value="index">
                        </el-option>
                    </el-select>
                    <!-- Pannellum容器 -->
                ...
                <el-empty v-else description="无全景图片"></el-empty>
            </div>
        </el-drawer>
...
import ThreeHouseViewer from "@/components/ThreeHouseViewer.vue"
...
export default {
    ...
    data() {
        return {
            ...
            panoramaList: [], // 全景图片列表
            currentPanoramaIndex: 0, // 当前全景索引
            drawerPanorama: false, // 360度全景抽屉
            drawer3DViewer: false, // 真3D模型漫游抽屉
    ...
    beforeDestroy() {
        ...
        watch: {
        // 新增：监听当前全景索引变化，重新加载Pannellum
        currentPanoramaIndex() {
            this.initPanorama();
        },
        // 新增：抽屉打开后初始化Pannellum
        drawerPanorama(val) {
            if (val) {
                this.$nextTick(() => {
                    this.initPanorama();
    ...
    methods: {
        // 打开真3D模型漫游抽屉
        open3DViewerDrawer() {
            this.drawer3DViewer = true;
        },
        // 关闭真3D模型漫游抽屉
        close3DViewerDrawer() {
            this.drawer3DViewer = false;
        },
        // 打开360度全景抽屉
        openPanoramaDrawer() {
            this.drawerPanorama = true;
        },
        // 关闭360度全景抽屉
        closePanoramaDrawer() {
            this.drawerPanorama = false;
        },

    initPanorama() {
    if (this.panoramaList.length === 0) return;
    const url = this.panoramaList[this.currentPanoramaIndex].url;
    ...
    pannellum.viewer('panorama', {
        ...
        autoLoad: true,
        showControls: true,
        autoRotate: 0.5,  // 可选：轻微自动旋转，便于查看
        hfov: 90,         // 水平视场角（默认100，调低更清晰）
        vaov: 90,         // 垂直视场角
        minHfov: 60,      // 最小水平视场（防止过度放大模糊）
        maxHfov: 120,     // 最大水平视场
        // 核心：提升质量参数
        renderer: 'webgl',  // 确保用 WebGL
        // 抗锯齿和清晰度
        antialias: true,
        // 性能与质量平衡
        multiRes: {
            basePath: url,  // 如果是多分辨率瓦片，改成你的图片
            path: '/%l/%s%y_%x',
            ...
            try {
                ...
                this.coverList = (data.covers || '')
                    ...
                    .map(url => ({
                     ...
                     this.panoramaList = JSON.parse(data.panoramaJson || '[]').map(item => ({
    uid: Date.now() + Math.floor(Math.random() * 1000),
    url: item.url,
    name: item.name || '未命名'
}));
        ...
        async downloadContract(fileName) {
            try {
                // 使用原生fetch避免axios拦截器问题
                const baseURL = this.$axios.defaults.baseURL;
                const url = `${baseURL}/file/getFile?fileName=${encodeURIComponent(fileName)}`;
