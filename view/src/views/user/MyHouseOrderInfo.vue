<template>
    <div class="container">
                            ...
                            <div class="operate-buttons">
                                <el-dropdown trigger="click" placement="bottom-end">
                                    ...
                                    <el-dropdown-menu slot="dropdown">
                                        <!-- 只有预约看房状态完成之后才需要服务评价 -->
                                        <el-dropdown-item v-if="scope.row.orderStatus === 2"
                                            @click.native="serviceEvaluations(scope.row)"
                                            icon="el-icon-document-checked">
                                            服务评价
                                        </el-dropdown-item>
                                        <el-dropdown-item v-if="scope.row.orderStatus === 0"
                                            @click.native="update(scope.row)" icon="el-icon-edit">
                                            修改
                                        </el-dropdown-item>
                                        <!-- 待看房状态下显示详情（查看核销码） -->
                                        <el-dropdown-item v-if="scope.row.orderStatus === 1"
                                            @click.native="update(scope.row)" icon="el-icon-view">
                                            详情
                                        </el-dropdown-item>
        ...
        <el-dialog title="预约看房记录" :show-close="false" :visible.sync="dialogVisible" :closeOnClickModal="false"
            ...
            <!-- 状态流转区域 -->
            <!-- 核销码显示区域 - 仅在待看房状态(1)显示 -->
            <div v-if="houseOrderInfo.orderStatus === 1" 
                 style="margin-top: 20px; padding: 15px; background-color: #f0f9eb; border: 1px solid #c2e7b0; border-radius: 4px; text-align: center;">
                <div style="font-size: 14px; color: #67c23a; margin-bottom: 10px;">
                    <i class="el-icon-finished"></i> 看房核销码 (请向房东/中介出示)
                </div>
                <div style="font-size: 24px; font-weight: bold; color: #67c23a; letter-spacing: 2px;">
                    {{ houseOrderInfo.writeOffCode || '加载中...' }}
                </div>
            </div>
            <div>
                <status-flow :status-records="houseOrderStatusList" />
            </div>
            <div v-if="houseOrderInfo.orderStatus === 2">
                <!-- 没有评价过,在此评价 -->
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <span v-if="houseOrderInfo.orderStatus === 0 || houseOrderInfo.orderStatus === 1" class="info-bt" @click="handleConfirmNo">
                    取消预约
                </span>
            </span>
        </el-dialog>
