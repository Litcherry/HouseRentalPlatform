...
public class LandlordServiceImpl extends ServiceImpl<LandlordMapper, Landlord> implements LandlordService {
    ...
    public Result<String> saveEntity(Landlord landlord) {
        ...
        // 校验身份证号的合法性，提供详细的错误信息
        String validationError = IdCardValidator.getValidationError(landlord.getIdcard());
     ...
     */
    @Override
    public Result<String> rejectApplication(Integer id, String reason) {
        // 删除该申请记录（打回相当于删除申请）
        Landlord landlord = this.getById(id);
        if (landlord == null) {
        ...
        // 如果已经审核通过，不能打回
        if (landlord.getIsAudit()) {
            return ApiResult.error("已审核通过的申请不能打回");
        }

        // 记录日志（可选）
        if (reason != null && !reason.trim().isEmpty()) {
            log.info("打回房东申请，ID: {}, 原因: {}", id, reason);
        }

        // 删除申请记录
        this.removeById(id);
        return ApiResult.success("申请已打回");
    }
