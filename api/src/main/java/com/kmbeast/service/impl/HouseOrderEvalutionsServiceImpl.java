...
public class HouseOrderEvalutionsServiceImpl extends ServiceImpl<HouseOrderEvaluationsMapper, HouseOrderEvaluations> implements HouseOrderEvaluationsService {
    ...
    @Resource
    private com.kmbeast.mapper.HouseMapper houseMapper;
    @Resource
    private com.kmbeast.mapper.UserMapper userMapper;
    ...
    public Result<String> saveEntity(HouseOrderEvaluations houseOrderEvaluations) {
        ...
        // 更新中介评分
        HouseOrderInfo orderInfo = houseOrderInfoMapper.selectById(houseOrderEvaluations.getHouseOrderInfoId());
        if(orderInfo != null && orderInfo.getHouseId() != null) {
            House house = houseMapper.selectById(orderInfo.getHouseId());
            if(house != null && house.getAgentId() != null) {
                User agent = userMapper.getUserById(house.getAgentId());
                if(agent != null) {
                    Integer currentCount = agent.getAgentScoreCount() == null ? 0 : agent.getAgentScoreCount();
                    Double currentScore = agent.getAgentScore() == null ? 5.0 : agent.getAgentScore();
                    
                    double totalScore = currentScore * currentCount + houseOrderEvaluations.getScore();
                    double newScore = totalScore / (currentCount + 1);
                    
                    BigDecimal bd = new BigDecimal(newScore).setScale(1, RoundingMode.HALF_UP);
                    
                    User updateAgent = new User();
                    updateAgent.setId(agent.getId());
                    updateAgent.setAgentScore(bd.doubleValue());
                    updateAgent.setAgentScoreCount(currentCount + 1);
                    userMapper.update(updateAgent);
                }
            }
        }
