...
import com.kmbeast.utils.PropertyCertificateValidator;
...
public class HouseServiceImpl extends ServiceImpl<HouseMapper, House> implements HouseService {
    ...
    public Result<String> update(House house) {
        System.out.println("=== 开始修改房屋 ===");
        System.out.println("房屋ID: " + house.getId());
        System.out.println("房屋名称: " + house.getName());
        System.out.println("房产证号: " + house.getPropertyCertificateNo());
        paramJudge(house, true); // true表示是修改操作
        updateById(house);
     ...
     */
    private void paramJudge(House house, boolean isUpdate) {
        ...
        // 房产证号校验 - 新增时必填，修改时如果有值则校验
        if (!isUpdate) {
            AssertUtils.hasText(house.getPropertyCertificateNo(), "房产证号不能为空");
        }
        // 只要有值，就必须符合格式
        if (house.getPropertyCertificateNo() != null && !house.getPropertyCertificateNo().trim().isEmpty()) {
            AssertUtils.isTrue(PropertyCertificateValidator.validate(house.getPropertyCertificateNo()),
                    "房产证号格式不正确，请填写正确的旧版《房屋所有权证》或新版《不动产权证书》编号");
        }
        // 名称长度显示
    ...
    public Result<String> saveEntity(House house) {
        paramJudge(house, false); // false表示是新增操作
        // 设置上当前新增的房东ID
...
//        System.out.println("=== 查询房屋详情 ===");
//        System.out.println("房屋ID: " + houseVO.getId());
//        System.out.println("房屋名称: " + houseVO.getName());
//        System.out.println("房产证号: " + houseVO.getPropertyCertificateNo());
...
//    }
    ...
    @Override
    public Result<HouseVO> selectById(Integer id) {
        House house = this.baseMapper.selectById(id);  // 先用 House 实体查询
        if (house == null) {
            return ApiResult.error("房屋不存在");
        }

        HouseVO houseVO = new HouseVO();
        BeanUtils.copyProperties(house, houseVO);  // 自动复制所有字段，包括 panorama！
